...
class BraveRewardsBrowserTest : public InProcessBrowserTest,
 ...
 public:
  ...
  void OnWalletInitialized(RewardsService* rewards_service, int result) {
    ASSERT_EQ(result, ledger::Result::WALLET_CREATED);
...
IN_PROC_BROWSER_TEST_F(BraveRewardsBrowserTest, AutoContribution) {
  ...
  // Check that tip table shows the appropriate contribution
  {
    content::EvalJsResult js_result = EvalJs(
        contents(),
        "const delay = t => new Promise(resolve => setTimeout(resolve, t));"
